...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex gap-2 flex-wrap">
            @if($bankReceipt->payment?->order)
                <a href="{{ route('admin.orders.edit', $bankReceipt->payment->order) }}" class="btn btn-secondary">
                    بازگشت به سفارش
                </a>
            @endif
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-5">
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body">
                    <p><strong>سفارش:</strong> #{{ $bankReceipt->payment->order?->id ?? '—' }}</p>
                    <p><strong>شماره پیگیری سفارش:</strong> {{ $bankReceipt->payment->order?->tracking_code ?? '—' }}</p>
                    <p><strong>مشتری:</strong> {{ $bankReceipt->payment->order?->user?->name ?? $bankReceipt->payment->order?->user?->email ?? '—' }}</p>
                    <p><strong>روش پرداخت:</strong> {{ $bankReceipt->payment->method?->title ?? '—' }}</p>
                    <p><strong>وضعیت رسید:</strong> {{ $bankReceipt->status }}</p>
                    <p><strong>وضعیت پرداخت:</strong> {{ __('messages.payment_statuses.' . $bankReceipt->payment->status) }}</p>
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <p class="text-muted small">
                            با تأیید این رسید، وضعیت پرداخت به «پرداخت‌شده» تغییر می‌کند و در صورت نیاز وضعیت سفارش نیز در فرایند بعدی قابل به‌روزرسانی است.
                        </p>
            ...
            @if($bankReceipt->status !== 'rejected')
